...
public class EmiCompat implements EmiPlugin {
	...
	private void registerGuiHandlers(EmiRegistry registry) {
		...
		registry.addDragDropHandler(MountedStorageScreen.class, new StorageGhostDragDropHandler<>());
		registry.addDragDropHandler(MountedStorageSettingsScreen.class, new SettingsGhostDragDropHandler<>());
	}

	private void registerRecipeHandlers(EmiRegistry registry) {
		registry.addRecipeHandler(ModContent.MOUNTED_STORAGE_CONTAINER_TYPE.get(), GridMenuInfo.crafting());
	}
}